...
patch(NavBar.prototype, {
    ...
    renderCustomOverlay() {
        ...
        if (this.state.isCustomHomeMenuOpen) {
            ...
            this.applyCustomHomeMenuBackground(overlayContainer);
            this.attachOverlayEventListeners(overlayContainer);
        }
    },

    getCustomHomeMenuBackground() {
        const company = user.activeCompany;
        return company?.custom_home_menu_background_color || '#e8d5f0';
    },

    applyCustomHomeMenuBackground(container) {
        const overlay = container.querySelector('.custom_home_menu_overlay');
        if (overlay) {
            overlay.style.setProperty(
                '--custom-home-menu-background',
                this.getCustomHomeMenuBackground()
            );
        }
    },
