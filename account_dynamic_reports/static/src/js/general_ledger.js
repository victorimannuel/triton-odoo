...
class GeneralLedger extends owl.Component {
    ...
    _amountClass(value) {
        ...
        if (!cleaned || Number.isNaN(number)) {
            return '';
        }
        if (number < 0) {
    ...
    _polishRenderedReport() {
        const root = this.tbody?.el?.closest('.o_tkg_general_ledger');
        if (!root) {
            return;
        }
        root.querySelectorAll('.amount-column, .text-right, .text-end').forEach((cell) => {
            this._applyAmountClass(cell);
            cell.querySelectorAll('span').forEach((node) => this._applyAmountClass(node));
        });
        root.querySelectorAll('tbody tr').forEach((row) => {
            ...
            row.addEventListener('click', (ev) => {
                if (ev.target.closest('.dropdown, .dropdown-menu, [data-bs-toggle="dropdown"], .btn-journal-items')) {
                    return;
                }
